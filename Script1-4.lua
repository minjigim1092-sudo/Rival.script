...
task.spawn(function()
    local repo_ms   = 'https://raw.githubusercontent.com/mstudio45/LinoriaLib/main/'
    local repo_orig = 'https://raw.githubusercontent.com/violin-suzutsuki/LinoriaLib/main/'

    local _libSrc, repo
    local ok_ms, src_ms = pcall(function() return game:HttpGet(repo_ms .. 'Library.lua') end)
    if ok_ms and src_ms and #src_ms > 100 then
        _libSrc = src_ms
        repo    = repo_ms
        print("[Rivals Hub] Using mstudio45 (mobile support)")
    else
        _libSrc = game:HttpGet(repo_orig .. 'Library.lua')
        repo    = repo_orig
        print("[Rivals Hub] Using violin-suzutsuki (fallback)")
    end

    _libSrc = _libSrc:gsub(
        "LocalPlayer:GetMouse%(%)[%s]*;?",
        "setmetatable({},{__index=function(_,k)local p=game:GetService('UserInputService'):GetMouseLocation();if k=='X'then return p.X elseif k=='Y'then return p.Y end end});"
    )
    local _gmCount = select(2, _libSrc:gsub("GetMouse%(%)",""))
    print("[Rivals Hub] GetMouse() remaining: " .. tostring(_gmCount))
    local Library      = loadstring(_libSrc)()
...
local _updateRot = nil
task.spawn(function()
    local ok, remote = pcall(function()
        return RS:WaitForChild("Remotes", 10)
            :WaitForChild("Replication", 10)
            :WaitForChild("Fighter", 10)
            :WaitForChild("UpdateCameraRotation", 10)
    end)
    if ok and remote then _updateRot = remote end
end)

local _vp = workspace.CurrentCamera.ViewportSize
local _scale = math.clamp(_vp.X / 1920, 0.38, 0.7)

Library.MinimumSize = Vector2.new(math.floor(480 * _scale), math.floor(600 * _scale))
...
local Window = Library:CreateWindow({
    ...
    Size               = math.floor(480 * _scale),
    TabPadding         = math.floor(6 * _scale),
    MenuFadeTime       = 0.1,
})
local Tabs = {
...
AimbotBox:AddToggle('AimbotEnabled', {Text='Aimbot', Default=false, Callback=function(v)
    if not _rivReady then return end
    env._riv_startAimbot(v)
end}):AddKeyPicker('AimbotKeybind', {Default='None', NoUI=false, Text='Aimbot', Mode='Hold', Callback=function(v)
    if not _rivReady then return end
    Toggles.AimbotEnabled:SetValue(v)
    env._riv_startAimbot(v)
end})
AimbotBox:AddDropdown('AimbotTargetPart', {Text='Target Part', Values={'Head','Torso','HumanoidRootPart'}, Default=1})
AimbotBox:AddDropdown('AimbotBlacklistPart', {Text='Blacklist Part', Values={'None','Head','Torso'}, Default=1})
AimbotBox:AddSlider('AimbotSmooth',  {Text='Smoothing', Default=10, Min=1,  Max=50, Rounding=0})
AimbotBox:AddSlider('AimbotFOV',     {Text='FOV',       Default=150, Min=10, Max=800, Rounding=0, Suffix='px'})
AimbotBox:AddToggle('AimbotShowFOV', {Text='Show FOV', Default=false})

local TriggerbotBox = Tabs.Combat:AddLeftGroupbox('Triggerbot')
TriggerbotBox:AddToggle('TriggerEnabled', {Text='Triggerbot', Default=false, Callback=function(v)
    if not _rivReady then return end
    env._riv_startTriggerbot(v)
end}):AddKeyPicker('TriggerKeybind', {Default='None', NoUI=false, Text='Triggerbot', Mode='Hold', Callback=function(v)
    if not _rivReady then return end
    Toggles.TriggerEnabled:SetValue(v)
    env._riv_startTriggerbot(v)
end})
TriggerbotBox:AddSlider('TriggerDelay',   {Text='Delay',    Default=0,   Min=0,  Max=500, Rounding=0, Suffix='ms'})
TriggerbotBox:AddSlider('TriggerFOV',     {Text='Set FOV',  Default=10,  Min=1,  Max=200, Rounding=0, Suffix='px'})
TriggerbotBox:AddSlider('TriggerScanFOV', {Text='Scan FOV', Default=30,  Min=1,  Max=400, Rounding=0, Suffix='px'})
TriggerbotBox:AddDropdown('TriggerBlacklistPart', {Text='Blacklist Part', Values={'None','Head','Torso'}, Default=1})
TriggerbotBox:AddToggle('TriggerShowFOV', {Text='Show FOV', Default=false})

local SilentAimBox = Tabs.Combat:AddRightGroupbox('Silent Aim')
SilentAimBox:AddToggle('SilentAim', {Text='Silent Aim', Default=false, Callback=function(v)
    if _rivReady then
        env._silentRageActive = v
        if v then env._tcAddConsumer() else env._tcRemoveConsumer() end
    end
end}):AddKeyPicker('SilentAimKeybind', {Default='None', NoUI=false, Text='Silent Aim', Mode='Hold', Callback=function(v)
    if not _rivReady then return end
    Toggles.SilentAim:SetValue(v)
    env._silentRageActive = v
    if v then env._tcAddConsumer() else env._tcRemoveConsumer() end
end})
SilentAimBox:AddDropdown('RageAimPart', {Text='Aim Part', Values={'Head','Torso'}, Default=1})
SilentAimBox:AddSlider('RageFOV', {Text='FOV', Default=250, Min=10, Max=800, Rounding=0, Suffix='px'})
SilentAimBox:AddToggle('SilentShowFOV', {Text='Show FOV', Default=false})

local RagebotBox = Tabs.Combat:AddRightGroupbox('Ragebot')
RagebotBox:AddToggle('RageEnabled', {Text='Ragebot', Default=false, Callback=function(v)
    if not _rivReady then return end
    env._riv_startRagebot(v)
end}):AddKeyPicker('RageKeybind', {Default='None', NoUI=false, Text='Ragebot', Mode='Hold', Callback=function(v)
    if not _rivReady then return end
    Toggles.RageEnabled:SetValue(v)
    env._riv_startRagebot(v)
end})
RagebotBox:AddToggle('VoidSpam', {Text='Void Spam', Default=false, Callback=function(v)
    if not _rivReady then return end
    env._riv_startVoidSpam(v)
end})
RagebotBox:AddDropdown('PreferredWeapon', {Text='Preferred Weapon', Values={'Primary','Secondary','Melee'}, Default=1, Callback=function(v)
    env._rivPreferredWeapon = v:lower()
end})
RagebotBox:AddToggle('VoidHide', {Text='Void Hide', Default=false, Callback=function(v)
    env._rivVoidHide = v
end})
RagebotBox:AddSlider('VoidHideTime',   {Text='Hide',   Default=2, Min=0.1, Max=5, Rounding=2, Suffix='s'})
RagebotBox:AddSlider('VoidAttackTime', {Text='Attack', Default=1, Min=0.1, Max=5, Rounding=2, Suffix='s'})

task.spawn(function()
    task.wait(0.5)
    local cont = RagebotBox.Container
    ...
    for _, ch in ipairs(cont:GetChildren()) do
        ...
        for _, lbl in ipairs(ch:GetDescendants()) do
            if lbl:IsA("TextLabel") then
                if lbl.Text == "Hide"   then hideEl = ch end
                if lbl.Text == "Attack" then atkEl  = ch end
            end
        end
    end
    if not hideEl or not atkEl then return end
    local baseOrder = hideEl.LayoutOrder

    local sliderH = math.max(hideEl.AbsoluteSize.Y, 38)

    local wrapper = Instance.new("Frame")
    wrapper.Name                   = "VoidHideSliders"
    wrapper.Size                   = UDim2.new(1, -8, 0, sliderH)
    wrapper.AutomaticSize          = Enum.AutomaticSize.None
    wrapper.BackgroundTransparency = 1
    wrapper.ClipsDescendants       = false
    wrapper.LayoutOrder            = baseOrder
    wrapper.Parent                 = cont

    atkEl.Parent       = wrapper
    atkEl.AnchorPoint  = Vector2.new(0, 0)
    atkEl.Position     = UDim2.new(0.5, 2, 0, 0)
    atkEl.Size         = UDim2.new(0.5, -2, 1, 0)

    hideEl.Parent      = wrapper
    hideEl.AnchorPoint = Vector2.new(0, 0)
    hideEl.Position    = UDim2.new(0, 0, 0, 0)
    hideEl.Size        = UDim2.new(0.5, -2, 1, 0)
end)

local WeaponBox = Tabs.Combat:AddLeftGroupbox('Weapon')
WeaponBox:AddToggle('Wallbang', {Text='Wallbang', Default=false, Callback=function(v)
    if not _rivReady then return end
    env._riv_startWallbang(v)
end})
WeaponBox:AddToggle('NoRecoil',  {Text='No Recoil / No Spread', Default=false, Callback=function(v)
    if _rivReady then env._rivNoRecoil = v end
end})
WeaponBox:AddToggle('RapidFire', {Text='Rapid Fire', Default=false, Callback=function(v)
    if _rivReady then env._rivRapidFire = v end
end})
WeaponBox:AddToggle('FastADS', {Text='Fast ADS', Default=false, Callback=function(v)
    if _rivReady then env._riv_startFastADS(v) end
end})
WeaponBox:AddToggle('NoEquipAnim', {Text='No Equip Animation', Default=false, Callback=function(v)
    if _rivReady then env._riv_startNoEquipAnim(v) end
end})
WeaponBox:AddToggle('HitsoundEnabled', {Text='Hitsound', Default=false, Callback=function(v)
    if _rivReady then env._riv_startHitsound(v) end
end})
WeaponBox:AddInput('HitsoundID', {Text='Sound ID', Default='rbxassetid://4764109000', Numeric=false, Finished=false})
WeaponBox:AddToggle('BigGun', {Text='Big Gun', Default=false, Callback=function(v)
    if _rivReady then env._riv_startBigGun(v) end
end})
WeaponBox:AddSlider('BigGunSize', {Text='Size Multiplier', Default=2, Min=0.1, Max=10, Rounding=1})

local ChecksBox = Tabs.Combat:AddRightGroupbox('Checks')
ChecksBox:AddToggle('TeamCheck',   {Text='Team Check',         Default=true})
ChecksBox:AddToggle('VisualCheck', {Text='Visual Check',       Default=false})
ChecksBox:AddToggle('AntiKatana',  {Text='Anti-Katana/Shield', Default=false})

local FovBox = Tabs.Player:AddLeftGroupbox('FOV Changer')
FovBox:AddSlider('FOVValue', {Text='Field of View', Default=70, Min=40, Max=120, Rounding=0, Suffix='°', Callback=function(v)
    Camera.FieldOfView = v
end})
FovBox:AddButton('Reset FOV', function() Camera.FieldOfView = 70; Options.FOVValue:SetValue(70) end)

local MovBox = Tabs.Player:AddLeftGroupbox('Movement')
MovBox:AddSlider('WalkSpeed', {Text='Walk Speed', Default=16, Min=16, Max=200, Rounding=0, Callback=function(v)
    local char = LP.Character
    local hum  = char and char:FindFirstChildWhichIsA("Humanoid")
    if hum then hum.WalkSpeed = v end
end})
MovBox:AddSlider('JumpPower', {Text='Jump Power', Default=50, Min=50, Max=300, Rounding=0, Callback=function(v)
    local char = LP.Character
    local hum  = char and char:FindFirstChildWhichIsA("Humanoid")
    if hum then hum.UseJumpPower = true; hum.JumpPower = v end
end})

local AntiEffBox = Tabs.Player:AddLeftGroupbox('Anti Effects')
AntiEffBox:AddToggle('AntiFlash', {Text='Anti Flash', Default=false, Callback=function(v)
    if _rivReady then env._riv_startAntiFlash(v) end
end})
AntiEffBox:AddToggle('NoSmoke', {Text='No Smoke', Default=false, Callback=function(v)
    if _rivReady then env._riv_startNoSmoke(v) end
end})

local FinisherBox = Tabs.Player:AddLeftGroupbox('Finisher')
FinisherBox:AddToggle('FinisherChanger', {Text='Finisher Changer', Default=false, Callback=function(v)
    if _rivReady then env._riv_startFinisherChanger(v) end
end})
FinisherBox:AddInput('FinisherName', {Text='Finisher Name', Default='Chark Attack', Numeric=false, Finished=false})

local TPBox = Tabs.Player:AddLeftGroupbox('Third Person')
TPBox:AddToggle('ThirdPerson', {Text='Third Person', Default=false, Callback=function(v)
    if not _rivReady then return end
    if v then Toggles.UnlockMouse:SetValue(false) end
    env._riv_startTP(v)
end}):AddKeyPicker('TPKeybind', {Default='None', NoUI=false, Text='Third Person', Mode='Hold', Callback=function(v)
    if not _rivReady then return end
    Toggles.ThirdPerson:SetValue(v)
    env._riv_startTP(v)
end})
TPBox:AddToggle('UnlockMouse', {Text='Unlock Mouse', Default=false, Callback=function(v)
    if not _rivReady then return end
    if v then Toggles.ThirdPerson:SetValue(false) end
    env._riv_startUnlockMouse(v)
end})

local RespawnBox = Tabs.Player:AddLeftGroupbox('Auto Respawn')
RespawnBox:AddToggle('AutoRespawn', {Text='Auto Respawn', Default=false, Callback=function(v)
    if not _rivReady then return end
    env._riv_startAutoRespawn(v)
end})
RespawnBox:AddSlider('RespawnDelay', {Text='Delay', Default=0, Min=0, Max=5, Rounding=1, Suffix='s'})

local NoclipBox = Tabs.Player:AddLeftGroupbox('Noclip')
NoclipBox:AddToggle('NoclipEnabled', {Text='Noclip', Default=false, Callback=function(v)
    if not _rivReady then return end
    env._riv_startNoclip(v)
end})

local AABox = Tabs.Player:AddRightGroupbox('Anti-Aim')
AABox:AddToggle('AntiAim', {Text='Anti-Aim', Default=false, Callback=function(v)
    if not _rivReady then return end
    env._riv_startAA(v)
end}):AddKeyPicker('AAKeybind', {Default='None', NoUI=false, Text='Anti-Aim', Mode='Hold', Callback=function(v)
    if not _rivReady then return end
    Toggles.AntiAim:SetValue(v)
    env._riv_startAA(v)
end})
AABox:AddDropdown('AAMethod', {
    Text='Method',
    Values={'Spinbot', 'Backwards', 'Right', 'Left'},
    Default=2,
})
AABox:AddSlider('AASpinSpeed',    {Text='Spin Speed',    Default=15, Min=1,   Max=30,  Rounding=0, Suffix='x'})
AABox:AddSlider('AACustomAngle',  {Text='Custom Angle',  Default=0,  Min=-90, Max=90,  Rounding=0, Suffix='°'})
AABox:AddToggle('AAJitter',       {Text='Jitter',        Default=false})
AABox:AddSlider('AAJitterRange',  {Text='Jitter Range',  Default=35, Min=0,   Max=180, Rounding=0, Suffix='°'})
AABox:AddToggle('AAPitchEnabled', {Text='Pitch',         Default=false})
AABox:AddSlider('AAPitchAngle',   {Text='Pitch Angle',   Default=0,  Min=-90, Max=90,  Rounding=0, Suffix='°'})

local FlyBox = Tabs.Player:AddRightGroupbox('Fly')
FlyBox:AddToggle('FlyEnabled', {Text='Fly', Default=false, Callback=function(v)
    if not _rivReady then return end
    env._riv_startFly(v)
end}):AddKeyPicker('FlyKeybind', {Default='F', NoUI=false, Text='Fly', Mode='Hold', Callback=function(v)
    if not _rivReady then return end
    Toggles.FlyEnabled:SetValue(v)
    env._riv_startFly(v)
end})
FlyBox:AddSlider('FlySpeed', {Text='Speed', Default=50, Min=10, Max=300, Rounding=0, Suffix=' st/s'})

local DevBox = Tabs.Player:AddRightGroupbox('Device Spoof')
DevBox:AddDropdown('DeviceSpoof', {Text='Device', Values={'None','Computer','Mobile','Console'}, Default=1})
DevBox:AddButton('Apply Spoof', function()
...
local TLBox = Tabs.Misc:AddLeftGroupbox('Target List')
TLBox:AddDropdown('TargetListMode', {
    Text='Mode',
...
local _tlDrop = TLBox:AddDropdown('TargetListPlayers', {
    Text='Player',
    Values=_tlGetPlayerNames(),
    Default=1,
})

local _tlListLabel = TLBox:AddLabel('목록: (비어있음)')
local function _tlUpdateLabel()
    local names = {}
    for n in pairs(env._targetList) do names[#names+1] = n end
    table.sort(names)
    _tlListLabel:SetText(#names == 0 and '목록: (비어있음)' or '목록: '..table.concat(names, ', '))
end

TLBox:AddButton('Add Selected', function()
    local name = Options.TargetListPlayers and Options.TargetListPlayers.Value
    if not name or name == '(없음)' then return end
    env._targetList[name] = true
    _tlUpdateLabel()
    Library:Notify('추가: '..name)
end)

TLBox:AddButton('Remove Selected', function()
    local name = Options.TargetListPlayers and Options.TargetListPlayers.Value
    if not name or name == '(없음)' then return end
    env._targetList[name] = nil
    _tlUpdateLabel()
    Library:Notify('제거: '..name)
end)

TLBox:AddButton('Refresh / Add All', function()
    local names = _tlGetPlayerNames()
    _tlDrop:SetValues(names)
    if #names > 0 and names[1] ~= '(없음)' then
        for _, n in ipairs(names) do env._targetList[n] = true end
        _tlUpdateLabel()
        Library:Notify('전체 추가 ('..#names..'명)')
    end
end)

TLBox:AddButton('Clear', function()
    env._targetList = {}
    _tlUpdateLabel()
    Library:Notify('목록 초기화')
end)

local SoundBox = Tabs.Misc:AddRightGroupbox('Sound Spammer')
SoundBox:AddToggle('SoundSpammer', {
    Text = 'Sound Spammer',
    Default = false,
    Callback = function(v) if _rivReady then env._riv_startSoundSpammer(v) end end,
})
SoundBox:AddSlider('SoundSpamInterval', {Text='Interval', Default=0.2, Min=0.05, Max=1.0, Rounding=2, Suffix='s'})

local CollectBox = Tabs.Misc:AddLeftGroupbox('Collect')
CollectBox:AddToggle('CollectHP',   {Text='Collect Health', Default=false, Callback=function(v)
    env._rivCollectHP = v
    if _rivReady then env._riv_startDrop(v or env._rivCollectAmmo) end
end})
CollectBox:AddToggle('CollectAmmo', {Text='Collect Ammo',  Default=false, Callback=function(v)
    env._rivCollectAmmo = v
    if _rivReady then env._riv_startDrop(v or env._rivCollectHP) end
end})

local DuelBanBox = Tabs.Misc:AddRightGroupbox('Duel Ban')
DuelBanBox:AddToggle('AutoBanEnabled', {
    Text = 'Auto Ban',
    Default = false,
    Callback = function(v) env._riv_startAutoBan(v) end,
})
DuelBanBox:AddDropdown('AutoBanTarget', {
    Text = 'Ban Target',
    Values = { 'Riot Shield + Katana', 'Riot Shield', 'Katana' },
    Default = 1,
})

local ProjTPBox = Tabs.Misc:AddLeftGroupbox('Projectile TP')
ProjTPBox:AddToggle('ProjTPEnabled', {
    Text = 'Projectile TP',
    Default = false,
    Callback = function(v)
        if not _rivReady then return end
...
ProjTPBox:AddDropdown('ProjTPTarget', {
    Text = 'Target',
...
ESPBox:AddToggle('ESPBox',      {Text='Box',         Default=false, Callback=function(v)
    if _rivReady then env._riv_startESP(v or _espAnyOn()) end
end})
ESPBox:AddToggle('ESPTracer',   {Text='Tracers',     Default=false, Callback=function(v)
    if _rivReady then env._riv_startESP(v or _espAnyOn()) end
end})
ESPBox:AddToggle('ESPName',     {Text='Names',       Default=true,  Callback=function(v)
    if _rivReady then env._riv_startESP(v or _espAnyOn()) end
end})
ESPBox:AddToggle('ESPHealth',   {Text='Health Bar',  Default=true,  Callback=function(v)
    if _rivReady then env._riv_startESP(v or _espAnyOn()) end
end})
ESPBox:AddToggle('ESPDist',     {Text='Distance',    Default=false, Callback=function(v)
    if _rivReady then env._riv_startESP(v or _espAnyOn()) end
end})
ESPBox:AddToggle('ESPSkeleton', {Text='Skeleton',    Default=false, Callback=function(v)
    if _rivReady then env._riv_startESP(v or _espAnyOn()) end
end})
ESPBox:AddToggle('ESPChams',    {Text='Chams',       Default=false, Callback=function(v)
    if _rivReady then env._riv_startChams(v) end
end})
ESPBox:AddLabel('Chams Fill'):AddColorPicker('ESPChamsColor', {Default=Color3.new(1,0,0)})
ESPBox:AddLabel('Chams Outline'):AddColorPicker('ESPChamsOutline', {Default=Color3.new(1,1,1)})

ESPBox:AddToggle('LandmineESP', {Text='Landmine ESP', Default=false, Callback=function(v)
    if _rivReady then env._riv_startLandmineESP(v) end
end})
ESPBox:AddLabel('Landmine Color'):AddColorPicker('LandmineESPColor', {Default=Color3.fromRGB(255,80,80)})

local TargetBox = Tabs.Visuals:AddLeftGroupbox('Target')
TargetBox:AddToggle('TargetDot', {Text='Red Dot on Target', Default=false, Callback=function(v)
    if _rivReady then env._riv_startTargetDot(v) end
end})
TargetBox:AddToggle('TargetFOVCircle', {Text='FOV Circle', Default=false, Callback=function(v)
    if _rivReady then env._riv_startFOVCircle(v) end
end})
TargetBox:AddSlider('TargetFOVRadius', {Text='FOV Radius', Default=120, Min=10, Max=400, Rounding=0})

local UIBox = Tabs.Visuals:AddRightGroupbox('Game UI')
local _KB_ENTRIES = {
    {'MenuKeybind',         'Menu',          false},
    {'AimbotKeybind',       'Aimbot',        'AimbotEnabled'},
    {'TriggerKeybind',      'Triggerbot',    'TriggerEnabled'},
    {'SilentAimKeybind',    'Silent Aim',    'SilentAim'},
    {'RageKeybind',         'Ragebot',       'RageEnabled'},

    {'TPKeybind',           'Third Person',  'ThirdPerson'},
    {'AAKeybind',           'Anti-Aim',      'AntiAim'},
    {'FlyKeybind',          'Fly',           'FlyEnabled'},
}
...
local function _kbUpdateRows()
    if not _kbFrame or not _kbFrame.Parent then return end
    for _, entry in ipairs(_KB_ENTRIES) do
        local pickerName, _, toggleName = entry[1], entry[2], entry[3]
        local r = _kbRows[pickerName]; if not r then continue end
        local kp     = Keypickers and Keypickers[pickerName]
        local keyStr = (kp and kp.Value and kp.Value ~= "") and ('['..kp.Value..']') or "[...]"
        r.key.Text   = keyStr
        if r.mode and kp then
            local kpMode = kp.Mode or (toggleName and 'Toggle' or 'Always')
            r.mode.Text = '('..kpMode..')'
        end
    end
end

local function _buildKeybindGui()
    if _kbFrame and _kbFrame.Parent then _kbFrame:Destroy() end
    if _kbHbConn then _kbHbConn:Disconnect(); _kbHbConn = nil end
    _kbRows = {}

    local th      = Library.Theme
    local cBg     = th.Background    or Color3.fromRGB(25, 25, 25)
    local cDark   = th.DarkContrast  or Color3.fromRGB(15, 15, 15)
    local cAccent = th.Accent        or Color3.fromRGB(103, 93, 190)
    local cText   = th.TextColor     or Color3.fromRGB(240, 240, 240)
    local cSub    = th.SubTextColor  or Color3.fromRGB(160, 160, 160)
    local cOutline= th.OutlineColor  or Color3.fromRGB(50, 50, 50)

    local parent = Library.ScreenGui or LP.PlayerGui

    local frame = Instance.new("Frame")
    frame.Name                 = "_RivKeybindFrame"
    frame.Size                 = UDim2.new(0, 210, 0, 0)
    frame.Position             = UDim2.new(0, 12, 0.5, -100)
    frame.BackgroundColor3     = cBg
    ...
    div.BackgroundColor3 = cOutline
    div.BorderSizePixel  = 0
    div.LayoutOrder      = 1

    for i, entry in ipairs(_KB_ENTRIES) do
        local pickerName, label, toggleName = entry[1], entry[2], entry[3]
        local kp     = Keypickers and Keypickers[pickerName]
        local keyStr = (kp and kp.Value and kp.Value ~= "") and ('['..kp.Value..']') or "[...]"
        ...
        keyLbl.BackgroundColor3     = cDark
        keyLbl.Text                 = keyStr
        keyLbl.TextColor3           = cText
        keyLbl.TextSize             = 11
        keyLbl.Font                 = Enum.Font.GothamBold
        keyLbl.TextXAlignment       = Enum.TextXAlignment.Center
        Instance.new("UICorner", keyLbl).CornerRadius = UDim.new(0, 4)
        local ks = Instance.new("UIStroke", keyLbl)
        ks.Color = cOutline; ks.Thickness = 1
        ...
        nameLbl.Text             = label
        nameLbl.TextColor3       = cSub
        nameLbl.TextSize         = 11
        nameLbl.Font             = Enum.Font.Gotham
        nameLbl.TextXAlignment   = Enum.TextXAlignment.Left

        local modeLbl = Instance.new("TextLabel", row)
        modeLbl.Size                 = UDim2.new(0, 56, 1, 0)
        modeLbl.Position             = UDim2.new(1, -58, 0, 0)
        modeLbl.BackgroundTransparency = 1
        modeLbl.TextSize             = 10
        modeLbl.Font                 = Enum.Font.Gotham
        modeLbl.TextXAlignment       = Enum.TextXAlignment.Right
        local kpMode = (kp and kp.Mode) or (toggleName and 'Toggle' or 'Always')
        modeLbl.Text       = '('..kpMode..')'
        modeLbl.TextColor3 = cSub

        _kbRows[pickerName] = {key=keyLbl, mode=modeLbl}
    end

    _kbFrame  = frame
    _kbHbConn = RunService.Heartbeat:Connect(_kbUpdateRows)
end

UIBox:AddToggle('ShowKeybinds', {Text='Show Keybinds', Default=true, Callback=function(v)
    if v then
        _buildKeybindGui()
    else
        if _kbHbConn then _kbHbConn:Disconnect(); _kbHbConn = nil end
        if _kbFrame and _kbFrame.Parent then _kbFrame:Destroy(); _kbFrame = nil end
    end
end})

local TracerBox = Tabs.Visuals:AddRightGroupbox('Bullet Tracers')
TracerBox:AddToggle('BulletTracerEnemy', {Text='Enemy Tracers', Default=false})
    :AddColorPicker('BulletTracerEnemyCol', {Text='Enemy Color', Default=Color3.new(1,0.2,0.2)})
TracerBox:AddToggle('BulletTracerLocal', {Text='Local Tracers', Default=false, Callback=function(v)
    if _rivReady then env._riv_startBulletTracer(v) end
end})    :AddColorPicker('BulletTracerLocalCol', {Text='Local Color', Default=Color3.new(0.2,0.8,1)})
TracerBox:AddSlider('BulletTracerDuration', {Text='Duration (s)', Default=0.3, Min=0.05, Max=2.0, Rounding=2, Suffix='s'})

local SetBox = Tabs.Settings:AddLeftGroupbox('Menu')
SetBox:AddLabel('Menu Toggle'):AddKeyPicker('MenuKeybind', {Default='Insert', NoUI=true, Text='Menu keybind'})

local UnloadBox = Tabs.Settings:AddLeftGroupbox('Unload')
UnloadBox:AddButton('Unload Hub', function()
    local _toggleKeys = {
    ...
    for _, k in ipairs(_toggleKeys) do
        if Toggles[k] then pcall(function() Toggles[k]:SetValue(false) end) end
    end
    local _stopFns = {
    ...
    Library:Notify('Rivals Hub 언로드 완료')
    task.wait(1)
    pcall(function() Library:Unload() end)
end)

local AutoBox = Tabs.Settings:AddLeftGroupbox('Autoload')
AutoBox:AddButton('Set as Autoload', function()
    local loaderScript = [[
...
if ok2 and src then
    ...
    if ok then
        Library:Notify('✅ Autoload 설정 완료! 다음 접속부터 자동 로드됩니다.')
    else
        Library:Notify('❌ 실패: ' .. tostring(err))
    end
end)
AutoBox:AddButton('Remove Autoload', function()
    pcall(function() delfile("autoexec/RivalsHub_autoload.lua") end)
    Library:Notify('🗑️ Autoload 제거됨')
end)
...
pcall(function()
    ...
    SaveManager:SetFolder('RivalsHub')
end)
...
local function _saveConfig(name)
    ...
    local ok, err = pcall(function()
        ...
        for k, v in pairs(Toggles) do
            ...
            if ok2 then data['T_'..k] = val end
        end
        for k, v in pairs(Options) do
            if k ~= 'CfgNewName' and k ~= 'CfgSelect' then
                local ok2, val = pcall(function() return v.Value end)
                if ok2 then data['O_'..k] = val end
            end
        end
    ...
    local ok, err = pcall(function()
        ...
        for k, v in pairs(data) do
            if k:sub(1,2) == 'T_' then
                local key = k:sub(3)
                if Toggles[key] then pcall(function() Toggles[key]:SetValue(v) end) end
            elseif k:sub(1,2) == 'O_' then
                local key = k:sub(3)
                if Options[key] and key ~= 'CfgNewName' and key ~= 'CfgSelect' then
                    pcall(function() Options[key]:SetValue(v) end)
                end
            end
        end
    end)
...
local CfgBox = Tabs.Settings:AddRightGroupbox('Config')

CfgBox:AddDropdown('CfgSelect', {Text='Config 선택', Values=_getCfgList(), Default=1})

CfgBox:AddButton('🔄 Refresh', function()
    local list = _getCfgList()
    pcall(function() Options.CfgSelect:SetValues(list) end)
    Library:Notify('목록 새로고침됨')
end)

CfgBox:AddButton('📂 Load Selected', function()
    local name = Options.CfgSelect and Options.CfgSelect.Value
    local ok, err = _loadConfig(name)
    if ok then Library:Notify('✅ 로드됨: '..name)
    else Library:Notify('❌ 로드 실패: '..tostring(err)) end
end)
...
CfgBox:AddInput('CfgNewName', {Text='새 Config 이름', Default='default'})
CfgBox:AddButton('💾 Save As', function()
    local name = Options.CfgNewName and Options.CfgNewName.Value or 'default'
    if name == '' then name = 'default' end
    local ok, err = _saveConfig(name)
    if ok then
        Library:Notify('✅ 저장됨: '..name)
        local list = _getCfgList()
        pcall(function() Options.CfgSelect:SetValues(list) end)
    else
        Library:Notify('❌ 저장 실패: '..tostring(err))
    end
end)

Library:SetWatermark('Rivals Hub  |  Loading...')
...
task.spawn(function()
    ...
    pcall(function() _statusLbl:SetText('✅ Ready') end)
    pcall(function() SaveManager:LoadAutoloadConfig() end)
    pcall(function() Library:SetWatermark('Rivals Hub  |  Insert = Menu') end)
...
task.delay(10, function()
    if not _rivReady then
        _rivReady = true
        pcall(function() _statusLbl:SetText('✅ Ready (fallback)') end)
        pcall(function() Library:SetWatermark('Rivals Hub  |  Insert = Menu') end)
